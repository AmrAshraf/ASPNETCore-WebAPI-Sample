            ...
            steps {
                script {
                    withSonarQubeEnv('SonarQube') {
                        ...
                        sh "apt-get update && apt-get install -y php${PHP_VERSION} php${PHP_VERSION}-xml composer"
                        
                        // Run Composer install
                        sh "composer install"
                        
                        // Run SonarQube analysis
                        sh "sonar-scanner -Dsonar.host.url=${SONARQUBE_URL} -Dsonar.login=${SONARQUBE_LOGIN}"
                    }
                }
            }
        }
